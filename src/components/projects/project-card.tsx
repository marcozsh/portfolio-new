...
export default function ProjectCard({
  ...
  return (
    <div className="flex flex-col items-center text-center lg:text-start lg:flex-row m-7 lg:m-0">
      <img src={pageImg} className="w-[650px]"/>
      <div className="pl-5 flex flex-col gap-8 justify-center">
        ...
        <div className="flex flex-row flex-wrap gap-3 justify-center lg:justify-start">
          {techs.map((tech, index) => (
            <img key={index} src={tech.img} className="rounded" />
          ))}
        </div>
      </div>
    </div>
